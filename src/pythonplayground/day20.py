...
class ArticleFetcher:
    ...
    def fetch(self, url, fetch_all_pages) -> None:
        ...
        for card in doc.select(".card"):
            ...
            #emoji
            emoji_text = ""
            emoji_element = card.select_one(".emoji")
            if emoji_element is not None:
                emoji_text = emoji_element.text

            # content
            content_text = ""
            content_element = card.select_one(".card-text")
            if content_element is not None:
                content_text = content_element.text

            # title
            title_text = ""
            title_element = card.select(".card-title span")[1]
...
fetcher.fetch(
    url="https://python.beispiel.programmierenlernen.io/index.php", fetch_all_pages=True
)
